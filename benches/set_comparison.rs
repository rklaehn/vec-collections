...
pub fn creation(c: &mut Criterion) {
    ...
    for i in (100u64..=1000).step_by(100) {
        ...
        group.bench_with_input(BenchmarkId::new("vs_create", i), &values, 
            |b, values| b.iter(|| vs_create(black_box(values))));

        group.bench_with_input(BenchmarkId::new("bs_create", i), &values, 
            |b, values| b.iter(|| bs_create(black_box(values))));

        group.bench_with_input(BenchmarkId::new("hs_create", i), &values, 
...
pub fn lookup(c: &mut Criterion) {
    ...
    for i in (100u64..=1000).step_by(100) {
        ...
        group.bench_with_input(BenchmarkId::new("vs_lookup", i), &(coll, &values[0..10]),
            |b, coll| b.iter(|| vs_contains(black_box(&coll.0), &coll.1)));

        let coll: BTreeSet<u64> = values.iter().cloned().collect();
        group.bench_with_input(BenchmarkId::new("bs_lookup", i), &(coll, &values[0..10]), 
        |b, coll| b.iter(|| bs_contains(black_box(&coll.0), &coll.1)));

        let coll: HashSet<u64> = values.iter().cloned().collect();
        group.bench_with_input(BenchmarkId::new("hs_lookup", i), &(coll, &values[0..10]), 
